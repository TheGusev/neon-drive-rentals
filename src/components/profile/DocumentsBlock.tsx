    ...
    <SectionCard
      ...
      action={
        <Button
          size="sm"
          variant="ghost"
          className="h-8 gap-1.5 rounded-full text-[#2f80ed] hover:bg-[#2f80ed]/10 hover:text-[#2f80ed]"
    ...
    >
      <ul className="space-y-2">
        {clientDocuments.map((doc) => {
          ...
          return (
            <li key={doc.id} className="flex items-center gap-3 rounded-2xl bg-white p-3 ring-1 ring-slate-100">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 text-slate-500">
                <Icon className="h-5 w-5" />
              </div>
              <div className="min-w-0 flex-1">
                <div className="text-sm font-medium text-slate-900">{typeLabel[doc.type]}</div>
