...
export async function sendMessage(input: {
  ...
  const rows = await query<Row>(
    `insert into messages (client_id, sender, body) values ($1, $2, $3)
...
export async function markThreadRead(clientId: string, reader: "client" | "admin"): Promise<void> {
  ...
  await query(
    `update messages set read_at = now() where client_id = $1 and sender = $2 and read_at is null`,
    [clientId, other],
  ).catch(() => undefined);
}
...
export async function countUnreadFor(clientId: string, reader: "client" | "admin"): Promise<number> {
  ...
  const rows = await query<{ n: string }>(
    `select count(*)::text as n from messages where client_id = $1 and sender = $2 and read_at is null`,
    [clientId, other],
  );
